...
func Deserialize(json []byte, resource *core.Resource) error {
...
func (d *deserializeState) errInvalidSyntax(msg string, args ...interface{}) error {
	return core.Errors.InvalidSyntax(fmt.Sprintf("failed to parse json: "+msg+" (idx: %d)", append(args, d.off)))
}

func (d *deserializeState) errInvalidValue(msg string, args ...interface{}) error {
	return core.Errors.InvalidValue(fmt.Sprintf("failed to parse json: "+msg+" (idx: %d)", append(args, d.off)))
}
...
kvs:
	for d.opCode != scanEndObject {
		...
		)
		{
			...
			prop, err = d.navigator.Focus(attrName)
			if err != nil {
				return err
			}
		}
...
elements:
	for d.opCode != scanEndArray {
		...
		_, err := d.navigator.Focus(d.navigator.NewPrototype())
		if err != nil {
			return err
		}
...
func (d *deserializeState) parseStringProperty() error {
	prop := d.navigator.Current()

	// check property type
	if prop.Attribute().MultiValued || !prop.Attribute().IsStringBasedType() {
		return d.errInvalidSyntax("expects string based property for '%s'", prop.Attribute().DisplayName())
	}
	...
	if d.isNull(start, end) {
		return d.navigator.Delete()
	}

	if d.data[start] != '"' || d.data[end-1] != '"' {
		return d.errInvalidSyntax("expects string literal value for '%s'", prop.Attribute().DisplayName())
	}

	return d.navigator.Replace(string(d.data[start+1 : end-1]))
}
...
func (d *deserializeState) parseIntegerProperty() error {
	...
	if prop.Attribute().MultiValued || prop.Attribute().Type != core.TypeInteger {
		return d.errInvalidSyntax("expects integer property for '%s'", prop.Attribute().DisplayName())
	}
	...
	if err != nil {
		return core.Errors.InvalidValue("expects integer value")
	}

	return d.navigator.Replace(val)
}
...
func (d *deserializeState) parseBooleanProperty() error {
	...
	if prop.Attribute().MultiValued || prop.Attribute().Type != core.TypeBoolean {
		return d.errInvalidSyntax("expects decimal property for '%s'", prop.Attribute().DisplayName())
	}
	...
	if d.isTrue(start, end) {
		return d.navigator.Replace(true)
	} else if d.isFalse(start, end) {
		return d.navigator.Replace(false)
	} else {
...
func (d *deserializeState) parseDecimalProperty() error {
	...
	if err != nil {
		return core.Errors.InvalidValue("expects decimal value")
	}
