...
// events reported by the filter scanner, to be consumer by the filter compiler.
const (
...
//	                primary true
//
func compileFilter(filter string, urnDict *trie) (*exprNode, error) {
	compiler := &filterCompiler{
		scan:    &filterScanner{},
		data:    append(copyOf(filter), 0, 0),
		off:     0,
		op:      scanFilterSkipSpace,
		opStack: make([]*exprNode, 0),
		rsStack: make([]*exprNode, 0),
		urnDict: urnDict,
	}
	compiler.scan.init()
	...
	for compiler.hasMore() {
		...
		case pushOpRightParenthesis:
			for {
				popped := compiler.popOperatorIf(func(top *exprNode) bool {
		...
		case pushOpInsufficientPriority:
			minPriority := getOpPriority(step.val)
			for {
				popped := compiler.popOperatorIf(func(top *exprNode) bool {
					return getOpPriority(top.val) >= minPriority
				})
				if popped != nil {
					// ignore error. we are sure it won't err
					_ = compiler.pushBuildResult(popped)
	...
	for len(compiler.opStack) > 0 {
		_ = compiler.pushBuildResult(compiler.popOperatorIf(func(top *exprNode) bool {
			return true
		}))
	}
...
func (c *filterCompiler) pushOperator(step *exprNode) int {
	...
	if step.isOperator() && len(c.opStack) > 0 {
		...
		if p.isOperator() {
			if getOpLeftAssociativity(step.val) && getOpPriority(step.val) <= getOpPriority(p.val) {
				return pushOpInsufficientPriority
			} else if getOpLeftAssociativity(step.val) && getOpPriority(step.val) < getOpPriority(p.val) {
				return pushOpInsufficientPriority
			}
		}
	}
...
func (c *filterCompiler) popOperatorIf(condition func(top *exprNode) bool) *exprNode {
	if len(c.opStack) == 0 {
		return nil
	}
...
func (c *filterCompiler) pushBuildResult(step *exprNode) error {
	...
	if step.isPath() {
		head, err := compilePath(step.val, c.urnDict)
		if err != nil {
	...
	// push back in.
	switch getOpCardinality(step.val) {
	case 1:
		{
...
func (c *filterCompiler) next() (*exprNode, error) {
...
// as op code.
func (c *filterCompiler) scanLiteral() (*exprNode, error) {
	start := c.off - 1
...
// as op code.
func (c *filterCompiler) scanOperator() (*exprNode, error) {
	start := c.off - 1
	end := c.scanWhile(scanFilterContinue)
	switch c.op {
...
func (c *filterCompiler) scanPath() (*exprNode, error) {
	start := c.off - 1
	end := c.scanWhile(scanFilterContinue)
	switch c.op {
...
func (c *filterCompiler) scanOperatorOrPath() (*exprNode, error) {
	start := c.off - 1
	end := c.scanWhile(scanFilterContinue)
	switch c.op {
...
type filterScanner struct {
	// step function for the next byte/char in the filter.
	step func(*filterScanner, byte) int
	// parenthesis level, goes up for ( and down for ), should be 0 in the end.
	parenLevel int
	// error incurred during the scanning.
	err error
	// number of bytes that has been scanned, assists in formulating error information.
	bytes int
}
...
// explainFilterEvent returns a string description for the given filterEvent to help debugging.
func explainFilterEvent(ev int) string {
	switch ev {
