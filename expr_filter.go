...
func (c *filterCompiler) pushOperator(step *Expr) int {
	...
	if step.IsOperator() && len(c.opStack) > 0 {
		...
		if p.IsOperator() {
			if isLeftAssociative(step.value) && opPriority(step.value) <= opPriority(p.value) {
				return pushOpInsufficientPriority
			} else if isLeftAssociative(step.value) && opPriority(step.value) < opPriority(p.value) {
				return pushOpInsufficientPriority
			}
		}
	}
...
func (c *filterCompiler) pushBuildResult(step *Expr) error {
	...
	// push back in.
	switch opCardinality(step.value) {
	case 1:
		{
