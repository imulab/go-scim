...
//	         value  "foo@bar.com"
//
func compilePath(path string, urnDict *trie) (*exprNode, error) {
	compiler := &pathCompiler{
		scan:    &pathScanner{urnDict: urnDict},
...
func (c *pathCompiler) next() (*exprNode, error) {
...
func (c *pathCompiler) scanStep() (*exprNode, error) {
...
func (c *pathCompiler) scanFilter() (*exprNode, error) {
	start := c.off
	end := c.skipWhile(scanPathContinue)
	switch c.op {
	case scanPathEndFilter, scanPathEnd:
		root, err := compileFilter(string(c.data[start:end]), c.urnDict)
		if err != nil {
			return nil, err
		}
		c.scanOne()
...
func (ps *pathScanner) stateBeginStep(scan *pathScanner, c byte) int {
	...
	match, ok := ps.urnDict.nextTrie(c)
	if ok {
