...
// filter, for example, <name.familyName> returns a structure like:
//
//		name -> familyName
//
// When the given path contains a filter, for example, <emails[value eq "foo@bar.com"].primary> returns a structure like:
//
//	   emails -> eq -> primary
//	            /  \
//	         value  "foo@bar.com"
...
func (ps *pathScanner) stateBeginStep(scan *pathScanner, c byte) int {
	...
	urnLookupLock.RLock()
	defer urnLookupLock.RUnlock()

	if match := urnLookup.next(c); match != nil {
...
func (ps *pathScanner) stateTryNamespaceStep(root *byteTrie) func(scan *pathScanner, c byte) int {
	return func(scan *pathScanner, c byte) int {
		urnLookupLock.RLock()
		defer urnLookupLock.RUnlock()

		if match := root.next(c); match != nil {
