...
type Property interface {
	// Attribute returns a non-nil attribute to describe this property.
	Attribute() *spec.Attribute
	// Raw return the property's value in Golang's native type. The type correspondence are:
	//
	// SCIM string (Go string); SCIM integer (Go int64); SCIM decimal (Go float64); SCIM boolean (Go bool);
	// SCIM dateTime (Go string); SCIM reference (Go string); SCIM binary (Go string);
	// SCIM complex (Go map[string]interface{}); SCIM multiValued (Go []interface{})
	//
	...
	IsUnassigned() bool
	// Dirty returns true if any of Add/Replace/Delete method was ever called. This method is necessary to distinguish
